...
export class Locator {
    ...
    public isNearChurchill(): boolean {
        ...
        return (
            state === PositionState.NearChurchill ||
    ...
    private tryInitializeIfNeeded(): void {
        if (
            this.canEverGeolocate &&
            (!this.settings.getData("location-permission").unwrap() as boolean)
        ) {
    ...
    private setPositionStateNearChurchill(
        ...
        const position = Locator.latLongToChurchillSpace(
            coords.latitude,
            coords.longitude,
        );
        ...
        if (this.positionState === PositionState.NearChurchill) {
            const distToLast = position.distanceTo(
                this.latestPosition.unwrap(),
            );
            if (distToLast < MOVEMENT_SENSITIVITY) {
                this.logger.log(
                    `got update, not moving dot (distance of ${distToLast})`,
                );
                return; // Did not move enough; probably just in one place with GPS noise
            } else {
                this.logger.log(
                    `got update, moving dot (distance of ${distToLast})`,
                );
            }
        }

        this.onUpdateState(
            PositionState.NearChurchill,
            Some(position),
            Some(accuracyRadius),
        );
    }
    ...
    private setPositionStateNotNearChurchill(): void {
        ...
        this.onUpdateState(
            PositionState.NotNearChurchill,
            this.latestPosition,
            this.latestAccuracyRadius,
        );
    }
    ...
    private setPositionStateUnknown(): void {
        if (this.positionState === PositionState.NearChurchill) {
            this.onUpdateState(
                PositionState.UnsureNearChurchill,
                this.latestPosition,
                this.latestAccuracyRadius,
            );
        } else {
    ...
    private static latLongToChurchillSpace(
        latitude: number,
        longitude: number,
    ): LatLng {
        ...
        const x =
            (63832500.0 / 311.0) * longitude +
            (65216250 / 311) * latitude +
            190395034993 / 24880;
        const y =
            (-44100000 / 311) * longitude +
     ...
     */
    private static latLongNearChurchill(
        latitude: number,
        longitude: number,
    ): boolean {
